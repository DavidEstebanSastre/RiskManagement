...
function evaluateCustomPageSettings(target, page, pageType) {
    ...
    if ('viewId' in target) {
        ...
        page.view = Object.assign(Object.assign(Object.assign({ id: target.viewId, name: target.viewName }, (target.viewType && { viewType: target.viewType })), (target.viewPath && { path: target.viewPath })), (target.path && { path: target.path }));
    }
        ...
        page.view = Object.assign(Object.assign(Object.assign(Object.assign({}, (target.id && { id: target.id })), (customPageName && { name: customPageName })), (target.name && { template: target.name })), (target['viewType'] && { viewType: target['viewType'] }));
    }
}
/**
...
function getSettings(routing) {
    ...
    if (config) {
        return Object.assign(Object.assign(Object.assign(Object.assign({}, (config.flexibleColumnLayout && { flexibleColumnLayout: config.flexibleColumnLayout })), (config.viewType && { viewType: config.viewType })), (config.viewPath && { path: config.viewPath })), (config.path && { path: config.path }));
    }
    return undefined;
}
...
class BaseAppProvider extends appProvider_1.AppProvider {
    constructor(manifest, schemas, templateType, _settings, logger) {
        ...
        super(manifest, schemas, getHome(manifest[ux_specification_types_1.ManifestSection.ui5].routing), getPagesAndConfigs(manifest, schemas, templateType, logger), ux_specification_types_1.FioriElementsVersion.v4, Object.assign(Object.assign({}, getSettings(manifest[ux_specification_types_1.ManifestSection.ui5].routing)), appSettings), logger);
