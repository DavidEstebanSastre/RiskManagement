import { AppProvider } from '../../../common/appProvider';
import { AppSettings } from '../../application';
import type { Files } from '../../../common';
import { TemplateType } from '../../../common';
import type { v2, Manifest } from '@sap/ux-specification-types';
import { v4, PageTypeV4 } from '@sap/ux-specification-types';
import type { ExtensionLogger } from '../../../..';
export declare enum Constants {
    OptionalQuery = ":?query:",
    Component = "sap/fe/AppComponent",
    Namespace = "sap.fe.templates"
}
export declare const unhandledControllers: Array<string>;
export declare type ConversionParameters = {
    target: v4.RoutingTargetOptions;
    pageType: PageTypeV4;
    manifest: Manifest;
    schemas: Files;
    logger: ExtensionLogger;
    page: v4.Page;
    id: string;
    templateType: TemplateType;
};
/**
 * Determines the Pages(app.json) using routing configuration from manifest and creates the configuration for each page
 * @param manifest - manifest.json of the app
 * @param schemas - list of JSON schemas of all pages
 */
export declare function getPagesAndConfigs(manifest: Manifest, schemas: Files, templateType: TemplateType, logger?: ExtensionLogger): v4.Pages;
/**
 * Get the id of the page that is to open when the application is started.
 * @param routing UI5 routing configuration
 */
export declare function getHome(routing: v4.SapUi5RoutingV4): string;
/**
 * Method which detects app settings from manifest.
 * Currently only some settings are detected - Flexible Column Layout, viewPath, viewType, path
 * @param routing UI5 routing configuration.
 * @return {AppSettings} Application settings.
 */
export declare function getSettings(routing: v4.SapUi5RoutingV4): AppSettings;
export declare function getPages(manifest: Manifest, targetName: string): v4.SapUiAppPagesV4;
export declare abstract class BaseAppProvider extends AppProvider {
    constructor(manifest: Manifest, schemas: Files, templateType: TemplateType, _settings?: v2.AppSettings | v4.AppSettings, logger?: ExtensionLogger);
}
//# sourceMappingURL=baseAppProvider.d.ts.map