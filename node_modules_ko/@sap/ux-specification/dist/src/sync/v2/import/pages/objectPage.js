...
function prepareFlexChanges(changes, entitySet, logger) {
    ...
    changes.forEach((changeString) => {
        ...
        try {
            ...
            if (change && ((_a = change.controlId) === null || _a === void 0 ? void 0 : _a.includes(ux_specification_types_1.v2.FE_TEMPLATE_V2_OBJECT_PAGE)) && ((_b = change.controlId) === null || _b === void 0 ? void 0 : _b.includes(entitySet))) {
...
function importFlexChangesOfTable(flexChanges, propertyKey, configPart, importParameters, sectionTitle, sectionId) {
    ...
    if (sectionTitle) {
        ...
        facetIds = [sectionId.replace(/@/g, ''), sectionId === null || sectionId === void 0 ? void 0 : sectionId.replace(/::@com.sap.vocabularies/g, common_1.VOCWITHSLASH)];
    }
...
function importFlexChanges(schemaDefinition, configPart, flexChanges, propertyKey, sectionId, elementId) {
    ...
    for (const selector in flexChanges) {
        ...
        //Control type match?
        if (flexChange.controlType !== ((_a = schemaDefinition.properties[flexChange.propertyId]) === null || _a === void 0 ? void 0 : _a[common_1.SchemaTag.controlType])) {
            continue;
        }
...
function fillActions(importParameters, actionProperties, configPart, configParameters, importTypeId, facetId) {
    var _a;
    configPart[common_1.PropertyName.actions] = {};
    ...
    if (configParameters.flexChanges && Object.keys(configParameters.flexChanges).length > 0) {
        ...
        for (const actionProperty in actionProperties) {
            ...
            if (actionId) {
                configPart[common_1.PropertyName.actions][configActionId] = configParameters.factory.createInstance(ux_specification_types_1.PageType.ObjectPage, actionType);
                transferSettingsAndFlexChanges(importParameters, configPart[common_1.PropertyName.actions][configActionId], importParameters.jsonSchema.definitions[actionType], configParameters, sectionPart, actionId);
            }
            if (((_a = configPart[common_1.PropertyName.actions]) === null || _a === void 0 ? void 0 : _a[configActionId]) &&
                Object.keys(configPart[common_1.PropertyName.actions][configActionId]).length === 0) {
                delete configPart[common_1.PropertyName.actions][configActionId];
            }
        }
    }
    if (Object.keys(configPart[common_1.PropertyName.actions]).length === 0) {
        delete configPart[common_1.PropertyName.actions];
    }
}
/**
...
function fillFields(importParameters, sectionPart, fieldProperties, configPart, configParameters) {
    ...
    for (const fieldProperty in fieldProperties) {
        ...
        const elementId = sectionPart.title.includes(common_1.UIVOCABULARYALPHADOT)
            ? `${sectionPart.id.replace(/@/g, '')}::${fieldId}`
            : `${sectionPart.title}::${fieldId}`;
        transferSettingsAndFlexChanges(importParameters, configPart[sectionPart.id]['form']['fields'][fieldProperty], importParameters.jsonSchema.definitions[common_1.DefinitionName.Field], configParameters, {}, elementId);
...
function prepareTableSchema(tableType, sectionPart, schemaDefinitions) {
    ...
    if (tableType === 'TreeTable' || tableType === 'AnalyticalTable' || tableType === 'GridTable') {
        ...
        tableSchema['properties'] = Object.assign(Object.assign({}, tableSchema['properties']), schemaDefinitions[`ObjectPageResponsiveTableWithInlineDelete<${sectionPart.id}>`]['properties']);
    }
    return tableSchema;
}
/**
...
function fillTableInConfig(importParameters, sectionPart, manifestSettings, configPart, configParameters) {
    ...
    if (manifestSection || configParameters.flexChanges) {
        ...
        configPart[sectionPart.id][common_1.PropertyName.table] = OPtable;
        ...
        transferSettingsAndFlexChanges(importParameters, configPart[sectionPart.id][common_1.PropertyName.table], tableSchema, configParameters, sectionPart);
        ...
        if (toolBarActions === null || toolBarActions === void 0 ? void 0 : toolBarActions.properties) {
            ...
            fillActions(importParameters, toolBarActions.properties, configPart[sectionPart.id][common_1.PropertyName.table]['toolBar'], configParameters, types_1.ActionTypeV2.OPToolBar, facetId);
        }
        if (Object.keys(configPart[sectionPart.id][common_1.PropertyName.table]['toolBar']).length === 0) {
            delete configPart[sectionPart.id][common_1.PropertyName.table]['toolBar'];
        }
    }
}
/**
...
function fillChartSection(importParameters, sectionPart, manifestSettings, configPart, configParameters) {
    ...
    configPart[sectionPart.id][common_1.PropertyName.chart] = configParameters.factory.createInstance(ux_specification_types_1.PageType.ObjectPage, 'ObjectPageChart');
    if (manifestSection || configParameters.flexChanges) {
        transferSettingsAndFlexChanges(importParameters, configPart[sectionPart.id], sectionPart.schema, configParameters, { title: sectionPart.title });
        transferSettingsAndFlexChanges(importParameters, configPart[sectionPart.id][common_1.PropertyName.chart], importParameters.jsonSchema.definitions[common_1.DefinitionName.ObjectPageChart], configParameters, sectionPart);
    }
}
/**
...
function fillConfig(importParameters, configParameters) {
    ...
    objectPageConfig.header = configParameters.factory.createInstance(ux_specification_types_1.PageType.ObjectPage, common_1.DefinitionName.ObjectPageHeader);
    const definitions = importParameters.jsonSchema.definitions;
    const schemaDefinition = definitions[common_1.DefinitionName.ObjectPageHeader]
        ? definitions[common_1.DefinitionName.ObjectPageHeader]
    ...
    const headerAction = definitions[common_1.DefinitionName.HeaderActions];
    if (headerAction) {
    ...
    const footerAction = definitions[common_1.DefinitionName.FooterActions];
    if (footerAction) {
    ...
    transferSettingsAndFlexChanges(importParameters, objectPageConfig.layout, definitions[common_1.DefinitionName.ObjectPageLayout], configParameters);
    ...
    const sections = definitions[common_1.DefinitionName.Sections];
    if (sections) {
