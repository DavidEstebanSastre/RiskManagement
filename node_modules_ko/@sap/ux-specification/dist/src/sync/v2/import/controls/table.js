...
function addFlexForTable(config, change, jsonSchema) {
    if (change.controlType.endsWith(types_1.DefinitionName.Table)) {
        config[change.propertyId] = change.newValue;
    }
    else if (change.controlType === ux_specification_types_1.ControlType.TableColumn) {
        const schemaProperty = (jsonSchema === null || jsonSchema === void 0 ? void 0 : jsonSchema.definitions[types_1.DefinitionName.TableColumn]).properties[change.propertyId];
        if (!schemaProperty) {
            return;
        }
        ...
        const colId = annotationPart[annotationPart.length - 1]
            ...
            .replace('::com.sap.vocabularies', types_1.VOCWITHCOLONS)
            .replace('/', ':2f');
        if (!config.columns) {
            config.columns = {};
        }
