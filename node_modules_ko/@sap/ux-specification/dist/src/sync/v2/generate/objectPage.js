...
function addFlexChangeForSection(appSchema, schemaSection, property, sectionType) {
    schemaSection.properties[property][common_1.SchemaTag.artifactType] = ux_specification_types_1.ArtifactType.FlexChange;
    let controlType;
    switch (sectionType) {
    ...
    schemaSection.properties[property][common_1.SchemaTag.controlType] = controlType;
}
/**
...
function addHeaderActionCallBackV2(entityType, appSchema, actionAnnotation, headerActions, oDataServiceAVT) {
    return (item, index) => {
        if (!item['Determining'] && item.$Type === "com.sap.vocabularies.UI.v1.DataFieldForAction" /* DataFieldForAction */) {
            ...
            appSchema.definitions[actionDefinitionKey][common_1.SchemaTag.annotationPath] = `/${convertedIdentification}/${index}`;
...
function addTableDefinition(tableType, facetId, facet, appSchema) {
    ...
    appSchema.definitions[schemaId][common_1.SchemaTag.annotationPath] = facet.annotationPath;
...
function handleLineItem(generateParameters, facetKey, appSchema, facet, facets, sectionType) {
    if (facet.base !== common_1.FacetBase.LineItem || !facetKey) {
        //no properties
        return;
    }
    const navigationProperty = facetKey.split('::')[0];
    const schemaIdForOpSection = `${sectionType}<${facetKey}>`;
    appSchema.definitions[schemaIdForOpSection] = common_1.createDefaultSection(appSchema, facet, facetKey);
    ...
    appSchemaSection.properties.table = JSON.parse(JSON.stringify(appSchema.definitions[types_1.SectionTypeV2.SectionTable]['properties'][common_1.PropertyName.table]));
    ...
    appSchemaSection[common_1.SchemaTag.annotationPath] = facet.annotationPath;
    ...
    for (const facetItem in facets) {
        if (facets[facetItem].base === common_1.FacetBase.LineItem && facetItem.includes(navigationProperty)) {
...
function handleSubSection(appSchema, definitionKey, facet, facetId, sectionType) {
    appSchema.definitions[definitionKey] = common_1.createDefaultSection(appSchema, facet, facetId);
    ...
    appSchemaSection.properties.subsections[common_1.SchemaTag.isViewNode] = true;
}
/**
...
function handleSectionChart(appSchema, definitionKey, facet, facetKey, sectionType) {
    appSchema.definitions[definitionKey] = common_1.createDefaultSection(appSchema, facet, facetKey);
    const appSchemaSection = appSchema.definitions[definitionKey];
...
function handleAppSchemaFormSection(appSchema, definitionKey, facet, facetKey, sectionType) {
    ...
    appSchemaSection[common_1.SchemaTag.annotationPath] = facet.annotationPath;
    appSchemaSection[common_1.SchemaTag.isViewNode] = true;
...
function getSectionType(facetType, isSubSection) {
    const sectionTypeMapping = new Map([
        [common_1.FacetBase.CollectionFacet, types_1.SectionTypeV2.Section],
        [common_1.FacetBase.LineItem, types_1.SectionTypeV2.SectionTable],
        [common_1.FacetBase.Chart, types_1.SectionTypeV2.SectionChart],
        [common_1.FacetBase.Address, types_1.SectionTypeV2.SectionAddress],
        [common_1.FacetBase.Contact, types_1.SectionTypeV2.SectionContact],
        [common_1.FacetBase.Form, types_1.SectionTypeV2.SectionForm],
        [common_1.FacetBase.Identification, types_1.SectionTypeV2.SectionForm]
    ]);
    const subSectionTypeMapping = new Map([
        [common_1.FacetBase.CollectionFacet, types_1.SectionTypeV2.SubSection],
        [common_1.FacetBase.LineItem, types_1.SectionTypeV2.SubSectionTable],
        [common_1.FacetBase.Chart, types_1.SectionTypeV2.SubSectionChart],
        [common_1.FacetBase.Address, types_1.SectionTypeV2.SubSectionAddress],
        [common_1.FacetBase.Contact, types_1.SectionTypeV2.SubSectionContact],
        [common_1.FacetBase.Form, types_1.SectionTypeV2.SubSectionForm],
        [common_1.FacetBase.Identification, types_1.SectionTypeV2.SubSectionForm]
    ]);
    if (isSubSection) {
...
function addSection(generateParameters, facets, facetKey, sections, appSchema, isSubSection = false) {
    ...
    switch (sectionType) {
        ...
        case types_1.SectionTypeV2.SectionForm:
        case types_1.SectionTypeV2.SubSectionForm: {
            const withActions = facet.base !== common_1.FacetBase.Identification;
        ...
        case types_1.SectionTypeV2.SectionContact: {
            common_1.handleAddressContact(facet, facetKey, sections, appSchema, ux_specification_types_1.SectionType.Section, undefined, 'V2');
            break;
        }
        case types_1.SectionTypeV2.SubSectionAddress:
        case types_1.SectionTypeV2.SubSectionContact: {
            common_1.handleAddressContact(facet, facetKey, sections, appSchema, ux_specification_types_1.SectionType.Section, ux_specification_types_1.SectionType.SubSection, 'V2');
            break;
        }
        default: {
...
function addSections(generateParameters, appSchema, entityType, entitySetName, pages) {
    ...
    if (entityType) {
        sections[common_1.SchemaTag.annotationPath] = `/${entityType.fullyQualifiedName}/@${"com.sap.vocabularies.UI.v1.Facets" /* Facets */}`;
    }
    if (!pages) {
        return;
    }
    // Get facet annotations
    const facets = common_1.getObjectPageFacets(entityType, generateParameters === null || generateParameters === void 0 ? void 0 : generateParameters.serviceAVT, ux_specification_types_1.FioriElementsVersion.v2, generateParameters.logger);
...
function generateObjectPageSchemaV2(generateParameters, genericSchema, entityTypeName) {
    ...
    appSchema.properties[common_1.PropertyName.sections] = {
    ...
    // Add sections
    appSchema.definitions[common_1.DefinitionName.Sections] = {
        type: 'object',
        properties: {},
    ...
    if (!appSchema.definitions.Sections) {
        if (appSchema.definitions.RelatedFacetKeys && appSchema.definitions.RelatedFacetKeys['oneOf'].length) {
            ...
            appSchema.definitions.Sections[common_1.SchemaTag.isViewNode] = true;
        }
        else {
            delete appSchema.properties[common_1.PropertyName.sections];
        }
    }
